...
def post_product():
    if request.method == 'POST':
        ...
        department = db.session.query(models.Department).filter_by(id=data.get('department_id')).first()
        if department:
            try:   
...
def put_product(id):
    if request.method == 'PUT':
        ...
        if product and department:
            ...
            product.department_id = data.get('department', product.department_id)
            db.session.commit()
            return jsonify({'message': f'Product {product.name} updated successfully', 'data': data}), 200
        else:
